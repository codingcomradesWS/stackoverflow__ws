...
def get_frequent_no_answer(topic):
    list_topic = topic.split()
    if len(list_topic) == 1:
        url = f'https://stackoverflow.com/questions/tagged/{topic}?sort=MostFrequent&filters=NoAnswers&edited=true'
    else:
        new_topic = f'{list_topic[0]}'
        list_topic.pop(0)
        for i in range(len(list_topic)):
            new_topic = new_topic + "%20" + list_topic[i]
        url = f'https://stackoverflow.com/questions/tagged/{new_topic}?sort=MostFrequent&filters=NoAnswers&edited=true'
    r = requests.get(url)
...
if __name__ == '__main__':
    print(get_frequent_no_answer('python'))
